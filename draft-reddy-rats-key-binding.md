...
If such a key is disclosed, for example through a side-channel attack that recovers the private key, an attacker can import it into another environment of the same class and request an EAT over it. That environment's AK is genuine, the `cnf` matches the disclosed key, and protocol-level proof of possession succeeds, since the attacker holds the private component. The `cnf` comparison therefore does not detect this case. If the `key-attributes` claim only asserts that the key is protected and non-exportable at the time of attestation, the check also passes, because the disclosed key is genuinely non-exportable from the attacker's environment.

The two environments differ in one respect: the attacker's environment imported the key rather than generating it. Detecting re-hosting therefore depends on the `key-attributes` claim asserting that the Subject Key was generated within the attested environment and has never existed outside it, not merely that it is non-exportable at the time of attestation. A conforming environment that imported the key will not assert this, so a Verifier that requires the key to have been generated within the attested environment rejects attributes stating only current protection.

These checks apply at attestation time. Once a private key compromise is known, the Subject Key's certificate needs to be revoked, after which Relying Parties that check the certificate status reject it.
